...
import 'domain/HabitLastRunData.dart';
import 'domain/HabitMaster.dart';
import 'domain/HabitRunData.dart';
import 'domain/ServiceLastRun.dart';
...
class HabitMasterService {
  ...
  Future<HabitMaster> scheduleHabit({
    ...
    if (applicable && !alreadyPresent) {
      ...
      //add reminder
      habit.reminder.forEach((rem) {
        var notifStarts = DateTime(
          DateTime.now().year,
          DateTime.now().month,
          DateTime.now().day,
          habit.reminder == null ? 0 : rem.hour,
          habit.reminder == null ? 0 : rem.minute,
        );
